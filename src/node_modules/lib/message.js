...
const EMPTY_TRANSACTION_ID = Buffer.alloc(constants.kStunTransactionIdLength, 0)

const EMPTY_BUFFER = Buffer.alloc(0)
const EMPTY_MESSAGE_INTEGRITY = Buffer.alloc(constants.kStunMessageIntegritySize, 0)
...
class StunMessage {
   ...
   *
   * @return {number} The number of an attributes in current message.
   */
  get count() {
   ...
   *
   * @param {number} type - attribute type.
   * @return {StunAttribute|undefined} instance of StunAttribute or undefined attribute doesn't exist
   */
  getAttribute(type) {
   ...
   * @return {boolean} The result of an operation.
   */
  addMessageIntegrity(key) {
    ...
    hmac.update(msg.slice(0, -constants.kStunMessageIntegrityLength))
  ...
  addFingerprint() {
    ...
    const crc32buf = msg.slice(0, -constants.kStunFingerprintLength)
   ...
   * Convert current message to the Buffer.
   *
   * @return {Buffer} encoded stun message
   */
  toBuffer() {
